...
if arch-chroot "$INSTALL_ROOT" su - "$CREATED_USER" -c "yay --version" &>/dev/null; then
    ...
    arch-chroot "$INSTALL_ROOT" su - "$CREATED_USER" -c \
        "yay -S --needed --noconfirm brave-bin obsidian eww satty enpass-bin howdy i3lock-color otf-eb-garamond colloid-icon-theme-git colloid-cursors-git yacreader" 2>/dev/null || true
    ...
    echo -e "  ${B}    yay -S brave-bin obsidian eww satty enpass-bin howdy i3lock-color otf-eb-garamond colloid-icon-theme-git colloid-cursors-git yacreader${R}"
fi
