...
if arch-chroot "$INSTALL_ROOT" su - "$CREATED_USER" -c "yay --version" &>/dev/null; then
    ...
    # ── AUR packages ──
    echo -e "  ${S}Installing AUR packages: brave-bin, obsidian, eww-wayland, satty, enpass-bin...${R}"
    arch-chroot "$INSTALL_ROOT" su - "$CREATED_USER" -c \
        "yay -S --needed --noconfirm brave-bin obsidian eww-wayland satty enpass-bin" 2>/dev/null || true
    ...
    echo -e "  ${B}    yay -S brave-bin obsidian eww-wayland satty enpass-bin${R}"
fi
